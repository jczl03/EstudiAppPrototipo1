...
public class ModeloPrototipo1 {
  ...
  public static void anadirTarea(Tarea nuevaTarea){
      if(Main.tareas().size()==0){
          Main.tareas().Push(nuevaTarea);
      }else{
          boolean x=true;
          for(int i=0;i<Main.tareas().size() && x;i++){
              if(Main.tareas().Get(i).compare(nuevaTarea)>0){
                  x=false;
                  Main.tareas().insert(i, nuevaTarea);
              }
          }
          if(x){
                Main.tareas().Push(nuevaTarea);
          ...
          System.out.println("");
            ...
            if(Main.tareas().size()==0){
                ...
                for(int i=0;i<Main.tareas().size();i++){
                    System.out.println((i+1)+"."+Main.tareas().Get(i).getNombre());
                }
                tarea=input.nextInt();
                input.nextLine();
                Main.tareas().Get(tarea-1).print();
            }
      }catch(Exception e){
          System.out.println("Ha ocurrido un error");
      }
  ...
  public static void verTareas(int id){
      if(Main.tareas().size()==0){
          System.out.println("No hay tareas");
      }else{
          int tarea;
  ...
  public static void MenuVerFlashCard(){
      if(Main.mazos().size()==0){
          ...
          if(opcion==0){
              for(int i=0;i<Main.mazos().size();i++){
                  Main.mazos().Get(i).print();
              }
          }else{
              opcion--;
              if(opcion>=0 && opcion<Main.mazos().size()){
                  Main.mazos().Get(opcion).print();
              }else{
                  System.out.println("Ingrese una opción válida");
              }
          }
  ...
  public static void menu(){
    ...
    while(opcion!=0){
        ...
        switch(opcion){
            ...
            case 2 -> crearTarea();
            case 3 -> menuVerTareas();
            case 4 -> menuEliminarTareas();
            case 5 -> crearMazo();
